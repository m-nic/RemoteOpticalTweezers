...
function makeFresnelCircle(xc, yc, r = 10, z = 1e-9, N = 512, wv = 6.33e-6) {

    if (!frensnelet) {
        frensnelet = cwrapArrays
        ('fresnelCircle', 'array', ['number', 'number', 'number', 'number', 'number'], {
            heapIn: 'HEAPF64',
            heapOut: 'HEAPF64',
            returnArraySize: N * N
        });
    ...
    return frensnelet(xc, yc, r, z, wv, N);
}

onmessage = ({i, xc, yc, r, z = 1e-9, wv = 6.33e-6, N = 512}) => {
    postMessage({i, resp: makeFresnelCircle(xc, yc, r, z, N, wv)});
};
